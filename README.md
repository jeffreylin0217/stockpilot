...
StockPilot is a **retail demand and inventory analytics project** using Python, SQL and DuckDB, with a documented Power BI reporting layer. It asks a practical business question: **what should a retail planner investigate before changing promotions or placing replenishment orders?**

![StockPilot synthetic sample analysis: demand trends, promotion associations and simulated planning scenarios](docs/images/sample-analysis.png)

*Reproducible Python analysis of the included sample, not a Power BI screenshot. Inventory categories are deliberately simulated.*

## Start here

- **Business analysis:** [sample findings and recommended investigations](docs/SAMPLE_FINDINGS.md).
- **SQL:** [four business questions](sql/business_questions.sql), covering store demand share, within-store promotion comparisons, variability and the planning review queue.
- **Reporting:** [Power BI model](powerbi/DATA_MODEL.md) and [manual report build guide](powerbi/REPORT_BUILD_GUIDE.md).
- **Engineering support:** [pipeline architecture](docs/ARCHITECTURE.md), [tests](tests/) and [verification audit](docs/AUDIT.md).

| Business question | Approach | Decision supported |
|---|---|---|
| Where is demand concentrated or changing? | Store share, observed-day averages and indexed complete-week trends | Prioritize store/family review |
| Are promotions associated with higher demand? | Compare promoted and nonpromoted means; inspect store-level results | Investigate timing and store mix before changing promotions |
| Which demand is relatively variable? | Daily standard deviation divided by mean (CV) | Investigate spikes and assumptions behind buffers |
| Which replenishment scenarios need attention? | Trailing demand, assumed lead time and simulated stock | Create a review queue; verify real stock before ordering |

The emphasis is on explaining evidence and decisions. The local pipeline makes that analysis reproducible without requiring cloud infrastructure or advanced machine learning.
...
The README preview is included, so you can view it without installing plotting tools. To regenerate both the preview and its written findings:

```bash
pip install -r requirements-visuals.txt
python scripts/build_sample_brief.py
```

This optional script always analyzes the included sample in temporary storage; it does not replace your current processed data or Power BI exports.
...
## Folder structure
    ...
    ├── FILES.md
    ├── requirements.txt
    ├── requirements-visuals.txt    # optional preview dependency
    ├── pyproject.toml
    ├── data/
    │   ├── sample/
    │   ├── raw/
    ...
    ├── scripts/
    │   ├── generate_sample.py
    │   ├── build_sample_brief.py
    │   └── export_powerbi.py
    ├── sql/
    │   ├── bronze.sql
    │   ├── silver.sql
    │   ├── gold.sql
    │   └── business_questions.sql # standalone SELECT examples
    ├── src/stockpilot/
    │   ├── config.py
    │   ├── data_loader.py
    │   ├── validation.py
...
## Scope and evidence
The included repository uses **synthetic retail data**. Public retail data should only be described after a public source has been loaded and reviewed. The current project does not demonstrate real business impact, adoption, dollars saved, improved forecast accuracy, stockout reductions or stakeholder outcomes.
...
A future verified report can be saved at `powerbi/StockPilot.pbix` with actual screenshots under `powerbi/screenshots/`; neither is ignored. After building, reopening and validating the report, update the README PBIX status and record the completed manual checks in [docs/AUDIT.md](docs/AUDIT.md). Until then, the repository should continue to state that no completed PBIX/PBIT is included.
